...
def __int2bytes(numbytes, integer):
    if not 0 <= integer <= 2 ** (8 * numbytes):
        logging.warning("Integer is outside the allowable range",UserWarning)
...
def write(arguments):
    ...
    if len(message) > 96:
        logging.warning("Message is too long: " + len(message),UserWarning)
    ...
    if maxmessage <= 21:
        ...
        if maxresponse > 121:
            logging.warning("Response may be too long to receive.  Max possible length " + str(maxresponse),UserWarning)
    else:
        logging.warning("Message too long.  Message length " + str(len(message)),UserWarning)
        message = []
...
def __check_message(message):
    ...
    #checks checksum
    if checksum != 0:
        logging.warning("Checksum error",UserWarning)
        return []
...
def read(transmission):
    ...
    if startflag == csafe_dic.Extended_Frame_Start_Flag:
        ...
        logging.warning("No Start Flag found.",UserWarning)
        return []
    ...
    if not stopfound:
        logging.warning("No Stop Flag found.",UserWarning)
        return []
    ...
    while k < len(message):
        ...
        if abs(sum(msgprop[1])) != 0 and bytecount != abs(sum(msgprop[1])):
            logging.warning("Warning: bytecount is an unexpected length",UserWarning)
