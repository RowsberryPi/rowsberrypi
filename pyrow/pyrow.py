...
class pyrow(object):
    ...
    def __init__(self, erg):
        ...
        if sys.platform != 'win32':
            try:
                ...
                logging.exception("Exception detaching USB kernel driver")
    ...
    def get_monitor(self, forceplot=False,extrametrics=False):
        """
        ...
        command = ['CSAFE_PM_GET_WORKTIME',
                   'CSAFE_PM_GET_WORKDISTANCE', 'CSAFE_GETCADENCE_CMD',
                   'CSAFE_GETPOWER_CMD', 'CSAFE_GETCALORIES_CMD',
                   'CSAFE_GETHRCUR_CMD', 'CSAFE_PM_GET_WORKOUTINTERVALCOUNT']

        if forceplot:
            command.extend(['CSAFE_PM_GET_FORCEPLOTDATA', 32, 'CSAFE_PM_GET_STROKESTATE'])

        if extrametrics:
            command.extend(['CSAFE_PM_GET_STROKESTATS', 32, 'CSAFE_PM_GET_STROKESTATE'])
        ...
        if extrametrics:
            datapoints = results['CSAFE_PM_GET_STROKESTATS'][0] /2
            extrametrics = results['CSAFE_PM_GET_STROKESTATS'] 
            monitor['strokedistance'] = extrametrics[0]/100. # meter
            monitor['strokedrivetime'] = extrametrics[1]*10. # milliseconds
            monitor['strokerecoverytime'] = extrametrics[2]*10. # milliseconds
            monitor['strokelength'] = extrametrics[3]/100. # meters
            monitor['strokecount'] = extrametrics[4]
            monitor['strokepeakforce'] = extrametrics[5]/100. #N
            monitor['impulseforce'] = extrametrics[6]/100. # kg m/s
            monitor['strokeaverageforce'] = extrametrics[7]/100. # N
            monitor['workperstroke'] = extrametrics[8] # Joule
